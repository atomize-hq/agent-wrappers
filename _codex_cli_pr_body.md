...
Bring the Rust wrapper (`crates/codex`) into parity with upstream `codex` **rust-v0.103.0** by using the generated parity artifacts in `cli_manifests/codex/`.

This PR already contains:
- pinned upstream release assets (`cli_manifests/codex/artifacts.lock.json`)
- upstream CLI snapshots (`cli_manifests/codex/snapshots/0.103.0/**`)
- deterministic coverage reports (work queue) (`cli_manifests/codex/reports/0.103.0/**`)
- version metadata (`cli_manifests/codex/versions/0.103.0.json`)
...
- Upstream union snapshot: `cli_manifests/codex/snapshots/0.103.0/union.json`
- Coverage work queue: `cli_manifests/codex/reports/0.103.0/coverage.any.json`
- Per-target work queue(s): `cli_manifests/codex/reports/0.103.0/coverage.<target_triple>.json`
...
1) **Triage the delta for 0.103.0**
- Open `cli_manifests/codex/reports/0.103.0/coverage.any.json`
...
3) **Compare 0.103.0 to the current supported baseline**
...
- Determine what’s new/removed at the CLI surface layer by diffing the two union snapshots:
  - New surfaces = present in `snapshots/0.103.0/union.json` but not present in `snapshots/$BASELINE/union.json`
  - Removed surfaces = present in baseline union but not present in 0.103.0 union
...
- `cargo run -p xtask -- codex-report --version 0.103.0 --root cli_manifests/codex`
- `cargo run -p xtask -- codex-version-metadata --version 0.103.0 --status reported --root cli_manifests/codex`
...
- For the required target (`x86_64-unknown-linux-musl`), `cli_manifests/codex/reports/0.103.0/coverage.x86_64-unknown-linux-musl.json` has:
...
- If `snapshots/0.103.0/union.json.complete == true`, meet the same criterion for all expected targets.
...
- Primary report: `cli_manifests/codex/reports/0.103.0/coverage.x86_64-unknown-linux-musl.json`
...
- **Global flags model:** some flags are treated as global (root) and are reported at `path=[]` to avoid per-command noise; fix root coverage for global flags. If you see the same flag reported missing under many subcommands, confirm whether it is truly global for that version by inspecting `snapshots/0.103.0/union.json`.
...
Next: open `cli_manifests/codex/reports/0.103.0/coverage.x86_64-unknown-linux-musl.json` and work missing_commands → missing_flags → missing_args.
