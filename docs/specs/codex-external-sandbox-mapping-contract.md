...
- If the app-server rejects the pinned `approvalPolicy` / `sandbox` values, the backend MUST fail
  the run as `AgentWrapperError::Backend { message }` with a safe/redacted `message` (MUST NOT embed
  raw server output).
