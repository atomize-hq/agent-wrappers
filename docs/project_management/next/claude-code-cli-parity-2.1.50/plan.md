# claude-code-cli-parity-2.1.50 – Plan – Plan
...
- Docs/tasks/session_log edits happen only on the orchestration branch (`feat/claude-code-cli-parity-2.1.50`), never from worktrees.
...
- Orchestration branch: `feat/claude-code-cli-parity-2.1.50`.
- Branch naming: `claude-code-cli-parity-2-1-50-<triad>-v2-1-50-<role>` (e.g., `k-c0-schema-code`).
...
1. `git checkout feat/claude-code-cli-parity-2.1.50 && git pull --ff-only`
...
5. Create the task branch and worktree from `feat/claude-code-cli-parity-2.1.50`: `git worktree add -b <branch> wt/<branch> feat/claude-code-cli-parity-2.1.50`.
...
3. From outside the worktree, ensure the task branch contains the worktree commit (fast-forward if needed). Do **not** merge into `feat/claude-code-cli-parity-2.1.50`.
4. Checkout `feat/claude-code-cli-parity-2.1.50`; update `tasks.json` status; add an END entry to `session_log.md` with commands/results/blockers; create downstream prompts if missing; commit docs (`docs: finish <task-id>`).
...
4. Fast-forward merge the integration branch into `feat/claude-code-cli-parity-2.1.50`; update `tasks.json` and `session_log.md` with the END entry (commands/results/blockers); commit docs (`docs: finish <task-id>`).
...
- Version: `2.1.50`
- Coverage report: `cli_manifests/claude_code/reports/2.1.50/coverage.any.json`
