...
//!
//! It shells out to `codex exec`, enforces sensible defaults (non-interactive color handling,
//! timeouts, optional model selection), and supports both single-response and streaming flows.
//!
//! Surfaces:
...
//! - Point at a bundled Codex binary with [`CodexClientBuilder::binary`] or honor `CODEX_BINARY`
//!   and `PATH` fallbacks.
//! - Scope Codex data via [`CodexClientBuilder::codex_home`] and optionally create the layout with
//!   [`CodexClientBuilder::create_home_dirs`]; overrides are applied per spawn without mutating the
//!   parent environment.
//! - Use [`CodexHomeLayout`] to inspect `config.toml`, `auth.json`, `.credentials.json`,
//!   `history.jsonl`, `conversations/`, and `logs/` paths under an isolated home.
//!
...
//! - When `RUST_LOG` is unset, spawned commands inherit `RUST_LOG=error` to mute verbose tracing.
//! - Streaming apply/diff output (stdout/stderr/exit) flows into `file_change` events and any
//!   configured `json_event_log` tee before parsing.
//!
//! See `examples/json_stream.rs` and `crates/codex/EXAMPLES.md` for end-to-end walkthroughs.
...
impl CodexHomeLayout {
    ...
    pub fn materialize(&self, create_home_dirs: bool) -> Result<(), CodexError> {
        ...
        for path in [self.root(), conversations.as_path(), logs.as_path()] {
            std::fs::create_dir_all(path).map_err(|source| CodexError::PrepareCodexHome {
