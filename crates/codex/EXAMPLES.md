...
| `cargo run -p codex --example json_stream -- --output-last-message ./last_message.txt -- "Summarize repo status"` | `echo "Summarize repo status" \| codex exec --skip-git-repo-check --json --output-last-message ./last_message.txt` | Streams JSONL events live and reports where the last message file was written. |
