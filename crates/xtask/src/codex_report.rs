...
fn deterministic_rfc3339_now() -> String {
    if std::env::var("CI").is_ok() && std::env::var("SOURCE_DATE_EPOCH").is_err() {
        // CI/automation must be deterministic; require SOURCE_DATE_EPOCH.
...
fn build_report(
        ...
        for flag in &cmd.flags {
            ...
            if res.level.is_none() {
                if let Some(root) = find_inherited_iu_root(
                    ...
                    intentionally_unsupported.push(
                        ReportIntentionallyUnsupportedDeltaV1::Flag(ReportFlagDeltaV1 {
                            path: path.to_vec(),
                            key: flag.key.clone(),
                            upstream_available_on: flag.available_on.clone(),
                            wrapper_level: Some("intentionally_unsupported".to_string()),
    ...
    intentionally_unsupported.sort_by(|a, b| cmp_iu_delta(a, b));
...
fn cmp_iu_delta(
    ...
    let ar = iu_kind_rank(a);
    let br = iu_kind_rank(b);
    ar.cmp(&br).then_with(|| {
        cmp_path(iu_path(a), iu_path(b)).then_with(|| match (a, b) {
            (ReportIntentionallyUnsupportedDeltaV1::Flag(a), ReportIntentionallyUnsupportedDeltaV1::Flag(b)) => {
                a.key.cmp(&b.key)
            }
            (ReportIntentionallyUnsupportedDeltaV1::Arg(a), ReportIntentionallyUnsupportedDeltaV1::Arg(b)) => {
                a.name.cmp(&b.name)
            }
